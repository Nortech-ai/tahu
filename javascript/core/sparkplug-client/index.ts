 ...
 */
import * as sparkplug from "@nortech/sparkplug-payload";
import type { UPayload } from "@nortech/sparkplug-payload/lib/sparkplugbpayload";
...
interface SparkplugClient extends events.EventEmitter {
  ...
  on(
    event: "dcmd",
    listener: (device: string, payload: UPayload) => void
  ): this;
...
class SparkplugClient extends events.EventEmitter {
  ...
  stop(cb?: mqtt.CloseCallback, force?: boolean) {
    ...
    this.client!.end(force, undefined, cb);
  }
  ...
  private init() {
    ...
    this.client.on("message", (topic, message) => {
      let payload = maybeDecompressPayload(decodePayload(message)),
...
export function maybeDecompressPayload(payload: UPayload): UPayload {
...
export function decompressPayload(payload: UPayload): Uint8Array {
  let metrics = payload.metrics || [],
...
export function decodePayload(payload: Uint8Array | Reader): UPayload {
